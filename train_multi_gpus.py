...
def parse_args():
    ...
    parser.add_argument('--config', type=str, help='configuration file path',default="../configs/config_rodnet_hgwI_win16.py")
    parser.add_argument('--data_dir', type=str, default='prepare_dataset/data', help='directory to save trained model')
    #parser.add_argument('--resume_from', type=str, help='path to the trained model')
    parser.add_argument('--resume_from', type=str, help='path to the trained model')
    ...
    parser.add_argument('--use_ext_backbone', type=str, default=True,help='if use hgwi extension backbones')
...
if __name__ == "__main__":
    ...
    if model_cfg['type'] == 'CDC':
        rodnet = RODNet(n_class_train).to(device)
        criterion = nn.MSELoss()
    elif model_cfg['type'] == 'HG':
        rodnet = RODNet(n_class_train, stacked_num=stacked_num).to(device)
    ...
    for epoch in range(epoch_start, n_epoch):
        ...
        for iter, data_dict in enumerate(dataloader):
            ...
            confmap_preds = rodnet(data.float().to(device))

            loss_confmap = 0
            if stacked_num is not None:
                for i in range(stacked_num):
                    loss_cur = criterion(confmap_preds[i], confmap_gt.float().to(device))
                    loss_confmap += loss_cur
                loss_confmap.backward()
                optimizer.step()
            else:
                loss_confmap = criterion(confmap_preds, confmap_gt.float().to(device))
                loss_confmap.backward()
                optimizer.step()
