...
def prepare_data(dataset, config_dict, data_dir, split, save_dir, viz=False, overwrite=False):
    ...
    for seq in sets_seqs:
        seq_path = os.path.join(data_root, set_cfg['subdir'], seq).replace('\\','/')
        seq_anno_path = os.path.join(anno_root, set_cfg['subdir'], seq + config_dict['dataset_cfg']['anno_ext']).replace('\\','/')
        save_path = os.path.join(save_dir, seq + '.pkl').replace('\\','/')
...
if __name__ == "__main__":
    ...
    for split in splits:
        save_dir = os.path.join(out_data_dir, split).replace('\\','/')
