...
def parse_args():
    ...
    parser.add_argument('--config', type=str, default='./configs/config_rodnet_hgwi_win16.py',help='choose rodnet model configurations')
    parser.add_argument('--data_dir', type=str, default='./data/ROD2021/', help='directory to the prepared data')
    parser.add_argument('--checkpoint', type=str, default='./checkpoints/epoch_10_final.pkl',help='path to the saved trained model')
    parser.add_argument('--res_dir', type=str, default='./results/', help='directory to save testing results')
    parser.add_argument('--use_noise_channel', default=True, help="use noise channel or not")
    parser.add_argument('--demo',type=bool,default=True, help='False: test with GT, True: demo without GT')
...
if __name__ == "__main__":
    ...
    # checkpoint = OrderedDict()    # create new OrderedDict that does not contain `module.`
    ...
    #     rodnet.load_state_dict(new_state_dict)
    # else:
    ...
    for subset in seq_names:
        ...
        for iter, data_dict in enumerate(dataloader):
            ...
            save_path = os.path.join(test_res_dir, seq_name+'.txt').replace('\\','/')
            start_frame_name = image_paths[0].replace('\\','/').split('/')[-1].split('.')[0]
            end_frame_name = image_paths[-1].replace('\\','/').split('/')[-1].split('.')[0]
